...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // 测试当面付2.0生成支付二维码
     //System.out.println( main.test_trade_precreate());
     //main.test_trade_query();
    }
    ...
    // 测试当面付2.0查询订单
    public String test_trade_query(String numbersId) {
        // (必填) 商户订单号，通过此商户订单号查询当面付的交易状态
        String outTradeNo = numbersId;
    ...
    // 测试当面付2.0生成支付二维码
    public String test_trade_precreate(String numbersId,Float money) {
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
        String outTradeNo =numbersId;

        // (必填) 订单标题，粗略描述用户的支付目的。如“xxx品牌xxx门店当面付扫码消费”
        String subject = "德召文汽修";
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String imgName=String.format("qr-%s.png",
                        response.getOutTradeNo());
                String filePath = "D:\\saoma\\"+imgName;
                File file=new File("D:\\saoma");
                if(file.exists()) {
